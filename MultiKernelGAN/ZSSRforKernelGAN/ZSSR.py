...
class ZSSR:
    ...
    def extract_numbers_from_file(self, filename):
    ...
    def __init__(self, weights_dir_path, input_img_path, scale_factor=2, kernels=None, is_real_img=False, noise_scale=1.):
    ...
    def init_sess(self, init_weights=True):
        ...
        if init_weights:
            ...
            # Initialize weights            
            img_name = os.path.basename(self.input_img_path)
            img_num = self.extract_numbers_from_file(img_name)
            weights_path = self.weights_dir_path
            file_path = os.path.join(weights_path, "img"+str(img_num) + "/zssr_checkpoints/model.ckpt")
            file_to_load = os.path.join(weights_path, "img" + str(img_num) + "/zssr_checkpoints/checkpoint")
            ...
            if os.path.isfile(file_to_load):
                ...
                print("@@@@@ ZSSR weights loaded @@@@")
            else:
                print("File does not exist.")
                # save weights
                self.sess.run(self.init_op)
                self.saver.save(self.sess, file_path)
                print("@@@@@ ZSSR weights saved @@@@")
